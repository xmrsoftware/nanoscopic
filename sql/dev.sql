...
CREATE OR REPLACE FUNCTION get_blog_id_from_blog_title(IN _blog_title TEXT, OUT blog_id BIGINT) RETURNS BIGINT
    LANGUAGE SQL AS $$
...
CREATE OR REPLACE FUNCTION get_all_user_blog_pages(IN _blog_user_id BIGINT) RETURNS TABLE
    (blog_page_id BIGINT, blog_id BIGINT, blog_user_id BIGINT, blog_page_title TEXT, blog_page_header TEXT,
    blog_page_content TEXT, blog_page_meta_description TEXT, blog_page_url_slug TEXT,
    blog_page_show_on_front_page BOOLEAN)
    LANGUAGE SQL AS $$
        SELECT * FROM blog_page WHERE blog_user_id = _blog_user_id;
    $$;
...
CREATE OR REPLACE FUNCTION get_all_user_blog_posts(IN _blog_user_id BIGINT, IN _blog_id BIGINT) RETURNS TABLE
    (blog_post_id BIGINT, blog_user_id BIGINT, blog_id BIGINT, blog_post_title TEXT, blog_post_header TEXT,
    blog_post_content TEXT, blog_post_views BIGINT, blog_post_published TIMESTAMPTZ, blog_post_updated TIMESTAMPTZ,
    blog_post_url_slug TEXT, blog_post_meta_description TEXT, blog_post_free_content TEXT,
    blog_post_show_on_front_page BOOLEAN)
    LANGUAGE SQL AS $$
...
CREATE OR REPLACE FUNCTION get_comments_for_post(IN _blog_post_id BIGINT) RETURNS TABLE
    (blog_post_comment_id BIGINT, blog_user_id BIGINT, blog_post_id BIGINT, blog_id BIGINT,
    blog_post_comment_reply_to BIGINT, blog_post_comment_title TEXT,blog_post_comment_content TEXT,
    blog_post_comment_date_posted TIMESTAMPTZ, blog_post_comment_date_updated TIMESTAMPTZ)
    LANGUAGE SQL AS $$
...
CREATE OR REPLACE FUNCTION get_latest_blog_posts() RETURNS TABLE
    (blog_id BIGINT, blog_post_id BIGINT, blog_user_id BIGINT, blog_post_header TEXT, blog_post_free_content TEXT,
    blog_post_published TIMESTAMPTZ, blog_post_url_slug TEXT)
    LANGUAGE SQL AS $$
        SELECT blog_id, blog_post_id, blog_user_id, blog_post_header, blog_post_free_content, blog_post_published,
        blog_post_url_slug FROM blog_post ORDER BY blog_post_published DESC LIMIT 100;
    $$;
...
CREATE OR REPLACE FUNCTION check_if_email_verified(IN _user_id BIGINT) RETURNS BOOLEAN
    LANGUAGE SQL AS $$
        SELECT blog_user_email_verified FROM blog_user WHERE blog_user_id = _user_id;
    $$;

CREATE OR REPLACE FUNCTION check_if_username_exists(IN _username TEXT) RETURNS TEXT
    LANGUAGE SQL AS $$
        SELECT blog_user_username FROM blog_user WHERE blog_user_username = _username;
    $$;